// ==================================================
// ULTIMATE JORDAN ONLY BOOSTER - COMPLETE EDITION
...
var REGION_BLOCKER = {
  ...
  isBlocked: function(ip) {
    for (var country in this.middleEast) {
      var ranges = this.middleEast[country];
      for (var i=0; i<ranges.length; i++) {
        if (isInNet(ip, ranges[i][0], ranges[i][1])) {
          return {blocked: true, region: 'Middle East', country: country};
        }
      }
    }
    
    for (var country in this.europe) {
      var ranges = this.europe[country];
      for (var i=0; i<ranges.length; i++) {
        if (isInNet(ip, ranges[i][0], ranges[i][1])) {
          return {blocked: true, region: 'Europe', country: country};
        }
      }
    }
...
// ================= GEO ROUTE OPTIMIZER (JORDAN ONLY) =================
var GEO_OPTIMIZER = {
  regions: {
    'North': {
      ips: ['82.212.', '94.249.'],
...
var NET_MONITOR = {
  ...
  getQuality: function() {
    ...
    if (jordanRate > 80) score += 0;
    else if (jordanRate > 60) score -= 5;
    else if (jordanRate > 40) score -= 10;
    else score -= 20;
...
function debugReset() {
  ...
  return 'All systems reset - Ultimate Jordan Only Edition';
}
